...
function createButtons() {
  ...
  for (let i = 1; i < 5; i++) {
    ...
    button.position(10, 20 + (i + 1) * buttonSpacing);
    button.size(40, 30);
    button.mousePressed(() => {
      selectedFilter = i;
      butterflyFilter.hide();
...
function takePicture() {
  ...
  //gets content of photo from canvas based on the selected filter
  if (selectedFilter === 'hearts') {
    heartsFilter.drawHearts(); //draw hearts on the canvas
    photo.copy(get(0, 0, width / 2, (width / 2) * 0.75), 0, 0, width / 2, (width / 2) * 0.75, 0, 0, photoStripWidth / 3, (photoStripWidth / 3) / 1.33);
  } 
  else {
    photo.copy(get(0, 0, video.width, video.height), 0, 0, video.width, video.height, 0, 0, photoStripWidth / 3, (photoStripWidth / 3) / 1.33);
    applyFilter(photo, selectedFilter); //apply the selected filter
  }
...
class HeartsFilter {
  ...
  // draws hearts based on PoseNet head position
  drawHearts() {
    if (this.visible) {
